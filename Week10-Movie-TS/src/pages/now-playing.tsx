...
// 현재 상영중인 영화 데이터 가져오기
const fetchNowPlayingMovies = (page: number): Promise<MoviesResponse> =>
  axiosInstance
    .get(`/movie/now_playing?language=ko-KR&page=${page}`)
    .then((res) => ({
        results: res.data.results || [],
        total_pages: res.data.total_pages || 1, // 기본값 설정
      }));
...
const NowPlayingPage: React.FC = () => {
  ...
  const {
    data: movies = { results: [], total_pages: 1 },
    isLoading,
    isError,
    isFetching,
  } = useQuery<MoviesResponse, Error, MoviesResponse>({
    queryKey: ['now-playing-movies', page],
    queryFn: () => fetchNowPlayingMovies(page),
  });

  const isPreviousData = page > 1 && isFetching;
      ...
      <MovieGrid>
        {movies?.results?.map((movie) => (
          <Card
            ...
            posterPath={movie.poster_path ? `${IMAGE_BASE_URL}${movie.poster_path}` : ''}
