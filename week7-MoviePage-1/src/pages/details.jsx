...
const Details = () => {
    ...
    const { data: credits, isCreditLoading, isCreditError } = useQuery({
    ...
    if (!isLoading) {
        content =
            <>
                <Banner>
                    <BannerImg src={`https://image.tmdb.org/t/p/original/${movie?.poster_path}`} />
                    <Shading />
                    <TextBox>
                        <Text fs='2rem' fw='bold' mb='1rem'>{title}</Text>
                        <Text fs='0.8rem'>평균 &#9733; {movie?.vote_average.toFixed(1)}</Text>
                        <Text fs='0.8rem'>{movie?.release_date.substr(0, 4)}</Text>
                        <Text fs='0.8rem' mb='1rem'>{movie?.runtime}분</Text>
                        <Text fs='1.2rem' mb='1rem'>{movie?.tagline}</Text>
                        <Text fs='0.8rem'>{movie?.overview}</Text>
                    </TextBox>
                </Banner>
